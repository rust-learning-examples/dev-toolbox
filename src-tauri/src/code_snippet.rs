...
pub fn watch<E>(executor: E) where E: Fn(String) -> () + Send + Sync + 'static {
  ...
  loop {
    ...
    let global_keyboard_events_handler = KEYBOARD_EVENTS_HANDLER.lock().unwrap();
    let sleep_time = if global_keyboard_events_handler.allow_record || !prev_keys.is_empty() { 20 } else { 50 };
